...
void importFile(string fileName, vector<Element> * array)
{
	string inputLine;
	ifstream infile;
		 	...
		 	while(inputLine.substr(j,1).compare(")") != 0) // give temp the value of the string inside (...) AKA the priority
		  	...
		  	else if(temp != "" && isNumber(temp.substr(1,temp.length() - 1)) && temp != "-0") // ^# case. Exclude -0
		  	{
		  		...
		  		if(temp.substr(0,1) == "-") // we have a pure negative number
		  		{
		  			...
		  			e.numDays = y; // negative already taken care of
		  		}
...
int main(int argc, char* argv[])
{
	...
	int diff;
	stringstream geek(argv[3]); // convert string to int
	geek >> diff;
	//cout << "diff is " << diff << endl;

	if(argc == 4) // if a file was provided, import it
	{
		importFile((string)argv[1], &arr);
		importFile((string)argv[2], &arr1);
		...
		else
		{
			for(int i = 0; i < arr.size(); i++)
			{
				if(arr[i].taskDescr != arr1[i].taskDescr)
				{
					cout << arr[i].taskDescr << endl;
					cout << arr1[i].taskDescr << endl;
					cout << "False" << endl;
					return 0;
				}
				if(arr[i].hasNumPriority)
				{
					if(arr[i].numDays != (arr1[i].numDays - diff))
					{
						cout << i << endl;
						cout << arr[i].taskDescr << " " << arr1[i].taskDescr << endl;
						cout << arr[i].numDays << " " << arr1[i].numDays - diff << endl;
						cout << "False" << endl;
						return 0;
					}
				}
			    if(arr[i].newLine != arr1[i].newLine || arr[i].isHeader != arr1[i].isHeader || arr[i].priority != arr1[i].priority || arr[i].prefix != arr1[i].prefix)
				{
					cout << "Other problem" << endl;
					cout << arr[i].newLine << " " << arr1[i].newLine << endl;
					cout << arr[i].isHeader << " " << arr1[i].isHeader << endl;
					cout << arr[i].priority << " " << arr1[i].priority << endl;
					//cout << arr[i].breakPos << " " << arr1[i].breakPos << endl;
					cout << arr[i].prefix << " " << arr1[i].prefix << endl;
					cout << i << endl;
 					cout << "False" << endl;
					return 0;
				}
	...
	{
		cout << "Please provide updated file, old file, #updated" << endl;
		return 0;
	}
